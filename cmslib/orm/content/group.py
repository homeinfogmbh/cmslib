...
__all__ = ["GroupBaseChart", "GroupConfiguration", "GroupMenu", "MODELS"]
...
class _GroupContent(DSCMS4Model):
    ...
    def select(cls, *args, cascade: bool = False) -> Select:
        ...
        return (
            super()
            .select(cls, Group, Customer, Company, Address, *args)
            .join_from(cls, Group)
            .join(Customer)
            .join(Company)
...
class GroupBaseChart(_GroupContent):
    ...
    @classmethod
    def from_json(
        cls, json: dict, group: Group, base_chart: BaseChart, **kwargs
    ) -> GroupBaseChart:
...
class GroupMenu(_GroupContent):
    ...
    def select(cls, *args, cascade: bool = False) -> Select:
        ...
        return (
            super()
            .select(cls, Menu, menu_customer, menu_company, menu_address, *args)
            .join_from(cls, Menu)
            .join(menu_customer)
            .join(menu_company)
